...
import { Bot, Brain, Hand, Database, GitBranch, Layers, AlertTriangle, Gauge } from 'lucide-react';
...
<Cards>
  <Card title="Agent Classes" icon={<Bot />}>
...
<Cards>
  <Card title="Test All Branches" icon={<GitBranch />}>
    Ensure every if/else and try/catch is covered
  </Card>
  
  <Card title="Bulk Testing" icon={<Layers />}>
    Test with multiple records (200+) to ensure bulk operations work
  </Card>
  
  <Card title="Negative Testing" icon={<AlertTriangle />}>
